    ...
    if($UseAppTracking){
        ...
        if($returnedApps.data.ItemCount -eq 1){        
            ...
            if(-not $returnedApps.data.row.ows_Application_x0020_Package_x0020_) {
                ...
                $ApplicationScope = [string]$returnedApps.data.row.ows_Distribution_x0020_Scope_x0020__.trim(); write-Verbose "Application Scope - $ApplicationScope"
                ...
                if($ApplicationType -eq "MSI"){ #Find installer and uninstaller
                    if($ApplicationStatus.toupper() -eq "2.1.5 UAT PREP" -or $ApplicationStatus.ToUpper() -eq "2.1 ER/QA"){
            ...
            if(Get-CMApplication -Name $ApplicationTitle){
                ...
                $W8 = "Windows/All_x64_Windows_8.1_Client"
                $W8x86 = "Windows/All_x86_Windows_8.1_Client"
                $OSRequirements = @();
                $T1T2T3NotFoundYet = $true
                
                foreach($scope in $ApplicationScope.trim(";#").Split(";#")){
                    ...
                    if($scope -eq "Windows 8.1"){
                        $OSRequirements += $W8
                        $OSRequirements += $W8x86
                        write-verbose "Found $W8 requirement"
                    }
...
$appNames = "Adobe Acrobat Professional 11.0"
...
Remove-CMApplication -Name $appNames -Force
set-location c:\
#>